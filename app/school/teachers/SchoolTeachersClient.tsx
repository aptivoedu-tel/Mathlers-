...
interface SchoolTeachersClientProps {
  initialTeachers: SerializedTeacher[];
  domain: string;
}

export default function SchoolTeachersClient({ initialTeachers, domain }: SchoolTeachersClientProps) {
              ...
              {filteredTeachers.length > 0 ? (
                filteredTeachers.map((teacher) => (
                  <tr key={teacher._id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 font-semibold text-gray-900">{teacher.fullName}</td>
                    <td className="px-6 py-4 font-mono text-xs">{teacher.playerId}</td>
                    <td className="px-6 py-4 text-gray-500">{teacher.email}</td>
                    <td className="px-6 py-4 text-right">
                      <StatusChip variant={teacher.isActive ? 'success' : 'danger'}>
                ...
                <tr>
                  <td colSpan={4} className="px-6 py-12 text-center text-gray-500">
                    <EmptyState
                      ...
                      description={search ? "Try adjusting your search query." : "Start by adding a teacher individually or via bulk upload."}
                    />
                  </td>
                </tr>
              )}
