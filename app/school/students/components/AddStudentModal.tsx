...
export default function AddStudentModal({ domain, onClose, onSuccess }: AddStudentModalProps) {
  const [forms, setForms] = useState<StudentForm[]>([{ name: '', username: '', rollNumber: '', password: '' }]);
  ...
  const handleAddMore = () => {
    ...
    if (lastForm.rollNumber) {
      const numMatch = lastForm.rollNumber.match(/\d+$/);
      if (numMatch) {
        const numStr = numMatch[0];
        const num = parseInt(numStr, 10);
        const nextNum = (num + 1).toString().padStart(numStr.length, '0');
        nextRollNumber = lastForm.rollNumber.substring(0, lastForm.rollNumber.length - numStr.length) + nextNum;
      }
    }

    setForms([...forms, { name: '', username: '', rollNumber: nextRollNumber, password: '' }]);
  };
  ...
  const updateForm = (index: number, field: keyof StudentForm, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/school/students', {
        ...
        body: JSON.stringify({ students: forms }),
      });
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden outline-none">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="p-4 rounded-xl bg-blue-50/50 border border-blue-100 text-blue-800 text-sm">
            <p><strong>Note:</strong> Students will log in using their automatically generated email (e.g. <code>username.{domain}@mathlers.com</code>) and password. Roll Numbers are for identification only.</p>
          </div>

          <form id="add-students-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div key={index} className="p-5 rounded-2xl border border-gray-200 bg-white shadow-sm space-y-4 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-700 uppercase tracking-wide">Student Name</label>
                    <input
                      required
                      type="text"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-bold text-gray-700 uppercase tracking-wide">Roll Number</label>
                    <input
                      type="text"
                      placeholder="e.g. 1023"
                      value={form.rollNumber}
                      onChange={(e) => updateForm(index, 'rollNumber', e.target.value)}
